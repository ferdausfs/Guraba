...
import com.guraba.app.helpers.AlarmTasksSchedulingHelper.cancelBedtimeRoutineTasks
import com.guraba.app.helpers.AlarmTasksSchedulingHelper.cancelNotificationBatchTask
import com.guraba.app.helpers.AlarmTasksSchedulingHelper.scheduleBedtimeRoutineTasks
import com.guraba.app.helpers.AlarmTasksSchedulingHelper.scheduleNotificationBatchTask
import com.guraba.app.helpers.device.DeviceAppsHelper.getDeviceAppInfos
import com.guraba.app.helpers.device.NewActivitiesLaunchHelper
...
import com.guraba.app.helpers.usages.AppsUsageHelper.getAppsUsageForInterval
import com.guraba.app.models.AppRestriction
import com.guraba.app.models.BedtimeSchedule
import com.guraba.app.models.FocusSession
import com.guraba.app.models.Notification
import com.guraba.app.models.NotificationSettings
import com.guraba.app.models.RestrictionGroup
import com.guraba.app.services.notification.GurabaNotificationListenerService
import com.guraba.app.services.timer.EmergencyPauseService
import com.guraba.app.services.timer.FocusSessionService
import com.guraba.app.services.tracking.GurabaTrackerService
import com.guraba.app.services.vpn.GurabaVpnService
import com.guraba.app.utils.AppUtils
import com.guraba.app.utils.JsonUtils
...
class FgMethodCallHandler(
    ...
    private val vpnServiceConn =
        SafeServiceConnection(
            context = context,
            serviceClass = GurabaVpnService::class.java
        )

    private val notificationServiceConn =
        SafeServiceConnection(
            context = context,
            serviceClass = GurabaNotificationListenerService::class.java
        )
        ...
        when (call.method) {
            ...
            "activeEmergencyPause" -> {
                if (!Utils.isServiceRunning(context, EmergencyPauseService::class.java)
                    && Utils.isServiceRunning(context, GurabaTrackerService::class.java)
                ) {
                    context.startService(
                        Intent(context, EmergencyPauseService::class.java).setAction(
                            ServiceBinder.ACTION_START_GURABA_SERVICE
                        )
                    )
                    result.success(true)
                } else {
            ...
            "updateNotificationSettings" -> {
                ...
                if (notificationServiceConn.isActive) {
                    ...
                    notificationServiceConn.setOnConnectedCallback { service: GurabaNotificationListenerService ->
            ...
            "restartApp" -> {
                activity?.let {
                    NewActivitiesLaunchHelper.restartGuraba(it)
                }
                result.success(true)
            }
