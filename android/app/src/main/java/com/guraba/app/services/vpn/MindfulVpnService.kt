 ...
 *
 */
package com.guraba.app.services.vpn
 ...
 */
class GurabaVpnService : VpnService() {
    companion object {
        private const val TAG = "Guraba.VpnService"
    }

    private val mBinder = ServiceBinder(this@GurabaVpnService)
        ...
        get() = Runnable {
            try {
                DatagramChannel.open().use { tunnel ->
                    check(this@GurabaVpnService.protect(tunnel.socket())) { "Cannot protect the vpn socket tunnel" }
                    val serverAddress: SocketAddress = InetSocketAddress("localhost", 0)
                    tunnel.connect(serverAddress)
                    tunnel.configureBlocking(false)

                    val builder = this@GurabaVpnService.Builder()
                    ...
                    synchronized(this@GurabaVpnService) {
                ...
                SharedPrefsHelper.insertCrashLogToPrefs(this@GurabaVpnService, e)
                stopAndDisposeService()
            } catch (e: IOException) {
    ...
    override fun onBind(intent: Intent): IBinder? {
        return if (intent.action == ServiceBinder.ACTION_BIND_TO_GURABA) mBinder else null
    }
}
