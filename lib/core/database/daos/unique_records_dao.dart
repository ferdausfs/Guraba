...
class UniqueRecordsDao extends DatabaseAccessor<AppDatabase>
  ...
  /// Saves a single [GurabaSettings] object to the database.
  Future<void> saveGurabaSettings(GurabaSettings settings) async =>
      into(gurabaSettingsTable)
          .insert(settings, mode: InsertMode.insertOrReplace);

  /// Loads the first (and likely only) [GurabaSettings] object
  /// from the database. If none exists, returns default instance.
  Future<GurabaSettings> loadGurabaSettings() async =>
      await select(gurabaSettingsTable).getSingleOrNull() ??
      defaultGurabaSettingsModel;
