...
/// A Riverpod state notifier provider that manages [GurabaSettings].
final gurabaSettingsProvider =
    StateNotifierProvider<GurabaSettingsNotifier, GurabaSettings>(
  (ref) => GurabaSettingsNotifier(),
);

/// This class manages the state of guraba settings.
class GurabaSettingsNotifier extends StateNotifier<GurabaSettings> {
  GurabaSettingsNotifier() : super(defaultGurabaSettingsModel) {
  ...
  Future<GurabaSettings> init({bool addListenerToo = false}) async {
    final dao = DriftDbService.instance.driftDb.uniqueRecordsDao;
    state = await dao.loadGurabaSettings();
    ...
    if (addListenerToo) {
      ...
      Future.delayed(
        ...
        () => addListener(
          ...
          (state) => dao.saveGurabaSettings(state),
        ),
      );
    }
  ...
  void updateAppVersion() => state = state.copyWith(
        appVersion: MethodChannelService.instance.deviceInfo.gurabaVersion,
      );
}
